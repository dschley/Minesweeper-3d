...
public class Board {
	...
	public static boolean uncover(Board b, int x, int y, int z) {
		/*
		 * Initialize a queue If current square is non-mine uncover it and add
		 * to queue, otherwise gameover Remove a square from queue Count mines
		 * adjacent to it If adjacent mine count is zero, add any adjacent
		 * covered squares to queue and uncover them Go to step 3 if queue is
		 * not empty, otherwise finish
		 */

		if(b.board[x][y][z].visible){
			return true;
		}
	...
	public static void main(String[] args) {
		...
		distributeMines(b);
		/*
		 * printBoard(b); printBoardTiles(b);
		 */
